 ...
 * reassign leads whose prior sequence fully completed", or "don't
 * re-pitch the same service" on/off, or trigger a distribute-now run,
 * for every checked ICP in one submit instead of clicking each one
 * individually. Each ICP is still gated through IcpRepository's normal
 * per-ICP ownership/100%-links checks (bulkSetBoolColumn() and its
 * bulkSetAutoPush()/bulkSetRequireSequenceCompleted()/
 * bulkSetAvoidRepeatService() wrappers, and bulkDistribute() ->
